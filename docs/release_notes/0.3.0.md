...
- migrations:
   ...
   *2) Add `{app}/migrations/_xprez_refactor_0_3_0.py` with a processor for each custom module (required):*
    ``` py
    from xprez.migrations._refactor_0_3_0 import (
...
Internal reorganization (no API changes):
- Module components moved to `xprez/modules/` (model + config + form per module)
- `models/base/` contains framework abstracts
