...
def variables_css(request):
    ...
    default_module = Module.build()
    default_module_css_variables = default_module.get_css_variables()
    default_module_css_variables.update(
        default_module.build_config(0).get_css_variables()
    )
    module_current_css_variables = default_module_css_variables.copy()
    result += [
        ConfigParentMixin._format_css_rule(
    ...
    for breakpoint in breakpoints[1:]:
        config = default_module.build_config(breakpoint)
        config.css_breakpoint = breakpoint
        css = config.get_css_variables()
        changed_css_variables = ConfigParentMixin._diff_css_variables(
    ...
    for module_class in module_registry._registry.values():
        ...
        specific_current_css_variables = module.get_css_variables()
        specific_current_css_variables.update(config.get_css_variables())
        ...
        if changed_css_variables:
            result += [
                ConfigParentMixin._format_css_rule(
                    f".xprez-{module_class.module_css_class()}",
                    0,
